...
No `url` is stored — it's cheaply derivable from `id` (and `date` for nogi and sakura) via `@sakamichi-blog-archive/utils`'s `get*ScheduleEventUrl`/`getSakuraScheduleUrl` helpers whenever a consumer needs a link. Exception: `category_key: "birthday"` entries link to the member's profile page (`/artist/{member_uid}`) rather than an event detail page, for every group — so their link must be built from `member_uids` instead of `id`.
...
- `{group}/events.ics` — current + next JST month, `data/{group}/schedule/` events excluding `category_key: "birthday"` (birthdays), one event per occurrence. Each event's `member_uids` are resolved to unspaced names and set as the ics `DESCRIPTION` (`メンバー：{name} {name}...`), omitted when the event lists no members.
- `{group}/birthdays.ics` — one all-day birthday event per member for the current and next calendar year (not month-windowed, since a birthday only recurs once a year). Built **entirely from `@sakamichi-blog-archive/utils`'s member roster**, not from `data/{group}/schedule/` — the schedule data only holds `category_key: "birthday"` entries for months the fetcher has actually run for, which would leave the calendar full of gaps, whereas the roster carries every active member's birthdate. Titles are `🎂 {name}の{age}歳の誕生日` (unspaced name; age = event year − birth year), and each event links to the member's profile page. Members with no `birthdate` on record are skipped, and members with `graduatedAt` in the past (as of the run) are omitted entirely. Feb 29 birthdates are observed on Feb 28 in non-leap years.
